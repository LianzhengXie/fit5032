                ...
                <DataTable :value="submittedCards" tableStyle="min-width: 50rem">
                    ...
                    <Column field="isAustralian" header="Australian Resident">
                        <template #body="slotProps">
                            <span>{{ slotProps.data.isAustralian ? 'Yes' : 'No' }}</span>
                        </template>
                    </Column>
...
import ColumnGroup from 'primevue/columngroup';   // optional
  ...
  const validatePassword = (blur) => {
    ...
    if (password.length < minLength) {
        ...
        if (blur) errors.value.password = "Password must contain at least one case letter.";
    }else if (!hasNumber){
  ...
  const validateGender = (blur) =>{
    if (!formData.value.gender){
        errors.value.gender = blur ? "Gender selection is required." : errors.value.gender;
    }else {
  ...
  const validateReason = (blur) =>{
    if (!formData.value.reason){
        errors.value.reason = blur ? "Reason is required." : errors.value.reason;
    }else {
